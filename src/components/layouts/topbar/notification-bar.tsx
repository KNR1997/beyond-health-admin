...
import { SortOrder, UserNotification } from '@/types';
import {
  adminOnly,
  getAuthCredentials,
  hasAccess,
...
import { useNoticationRead, useNotificationsQuery } from '@/data/notification';

type IProps = { user: any };

const NotificationBar = ({ user }: IProps) => {
  ...
  const { readNotification } = useNoticationRead();
              ...
              <>
                ...
                <div>
                  {notifications.length ? (
                    notifications?.map((item: UserNotification) => {
                      const activeUser = Routes?.notification?.details(item?.id);
