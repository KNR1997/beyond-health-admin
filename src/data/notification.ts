...
export const useNoticationQuery = ({ id }: { id: string }) => {
  const { data, error, isLoading } = useQuery<UserNotification, Error>(
    [API_ENDPOINTS.NOTIFICATIONS, { id }],
    () => notificationClient.get({ slug: id }),
  );
...
export function useNoticationRead() {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useMutation(notificationClient.toggle, {
    onSuccess: () => {},
    // Always refetch after error or success:
    onSettled: () => {
      queryClient.invalidateQueries(API_ENDPOINTS.NOTIFICATIONS_IS_READ);
    },
  });
